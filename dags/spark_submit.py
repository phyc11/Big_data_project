...
df = spark.readStream \
    ...
    .option("subscribe", "weather_forecast_new") \
...
data_schema = StructType([
    ...
    StructField("actual_hour",TimestampType(),True)
])
...
query = df_selected.writeStream \
    .format("parquet") \
    .option("path", "hdfs://namenode:9000/data_from_API/weather_API_data") \
    .option("checkpointLocation", "hdfs://namenode:9000/data_from_API/checkpoint_API_data") \
    .option("header", "true") \
    .trigger(once=True) \
...
#     .outputMode("append") \
#     .format("es") \
#     .option("checkpointLocation", "hdfs://namenode:9000/tmp/checkpointES_new") \
#     .option("es.resource", "weather_forecast_big_data_new/_doc") \
#     .trigger(once=True) \
#     .start()
